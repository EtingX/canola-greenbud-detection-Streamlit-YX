...
def save_predictions_to_txt(prediction_result, image_path: str, output_txt_path: str):
    """
    将预测结果保存为 txt 文件（YOLO 格式）：
    格式：<class_id> <center_x> <center_y> <width> <height> <confidence>
    均为归一化数值（0-1）

    如果 prediction_result 拥有 attribute object_prediction_list，则使用它；
    否则假定 prediction_result 本身就是 ObjectPrediction 列表。
    注意：这里通过 pred.bbox.to_xyxy() 获取 [x1, y1, x2, y2]
    """
    from PIL import Image
    im = Image.open(image_path)
    img_w, img_h = im.size

    # 如果 prediction_result 没有 attribute object_prediction_list，则认为它就是列表
    if not hasattr(prediction_result, "object_prediction_list"):
        predictions = prediction_result
    else:
        predictions = prediction_result.object_prediction_list

    with open(output_txt_path, "w") as f:
        for pred in predictions:
            # 这里假设 pred.bbox 提供了 to_xyxy() 方法返回 [x1, y1, x2, y2]
            try:
                x1, y1, x2, y2 = pred.bbox.to_xyxy()
            except AttributeError:
                # 如果没有 to_xyxy() 方法，则直接尝试解包（如果 pred.bbox 是可迭代的）
...
def predict(
    ...
    for ind, image_path in enumerate(
        ...
        if export_txt == True:
            output_file = save_dir / "labels" / (filename_without_extension + '.txt')
            output_file.parent.mkdir(parents=True, exist_ok=True)
            save_predictions_to_txt(object_prediction_list, image_path, output_file)
